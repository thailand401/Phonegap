...
(function (global) {
  ...
  extend(fabric.StaticCanvas.prototype, /** @scope fabric.StaticCanvas.prototype */ {
    ...
    /**
     * Sets overlay image for this canvas0
   ...
   _initOptions: function (options) {
    ...
    renderAll: function (allOnTop) {
      ...
      if (this.overlayImage) {
        canvasToDrawOn.drawImage(this.overlayImage, this.overlayImageLeft, this.overlayImageTop,this.width, this.height);
      }
...
(function() {
 ...
 //this.contextTop.moveTo(p.x, p.y);
    //  this._reset();
     // this._addPoint(p);
 var pointerPoint = new fabric.Point(pointer.x, pointer.y);
 
     this._freeDrawingXPoints.push(pointerPoint.x);
     this._freeDrawingYPoints.push(pointerPoint.y);
	//  this.clearContext(this.contextTop);
	 // this.canvas.clearContext(this.contextTop);
 // this._points.push(point);
	...
	/*
      ...
      var p1 = this._freeDrawingXPoints[0];
      var p2 = this._freeDrawingYPoints[0];
      
      //if we only have 2 points in the path and they are the same
      //it means that the user only clicked the canvas without moving the mouse
      //then we should be drawing a dot. A path isn't drawn between two identical dots
      //that's why we set them apart a bit
      //if (this._freeDrawingYPoints.length === 2 && p1.x === p2.x && p1.y === p2.y) {
        //  p1 -= 0.5;
         // p2 += 0.5;
     // }
      ctx.moveTo(p1, p2);
     ...
     // for (var i = 1, len = this._points.length; i < len; i++) {
		...
		//var midPoint = new Point(this.x + (that.x - this.x)/2, this.y + (that.y - this.y)/2);
     ...
     // }
      ...
      var px1 = this._freeDrawingXPoints[0];
	  var py1 = this._freeDrawingYPoints[0];
      var px2 = this._freeDrawingXPoints[1];
	  var py2 = this._freeDrawingYPoints[1];
      
      //if we only have 2 points in the path and they are the same
      //it means that the user only clicked the canvas without moving the mouse
      //then we should be drawing a dot. A path isn't drawn between two identical dots
      //that's why we set them apart a bit
      if (this._freeDrawingXPoints.length === 2 && px1 === px2 && py1 === py2) {
          px1 -= 0.5;
          px2 += 0.5;
      }
      ctx.moveTo(px1, py1);

      for (var i = 1, len = this._freeDrawingXPoints.length; i < len; i++) {
        // we pick the point between pi+1 & pi+2 as the
        // end point and p1 as our control point.
		//new Point(this.x + (that.x - this.x)/2, this.y + (that.y - this.y)/2);
        var midPoint = new fabric.Point(px1 +(px2 - px1)/2, py1 +(py2-py1)/2);//p1.midPointFrom(p2);
        ctx.quadraticCurveTo(px1, py1, midPoint.x, midPoint.y);

        px1 = this._freeDrawingXPoints[i];
		py1 = this._freeDrawingYPoints[i];
		 px2 = this._freeDrawingXPoints[i+1];
		py2 = this._freeDrawingYPoints[i+1];
       // p2 = this._points[i+1];
      }
      // Draw last line as a straight line while
      // we wait for the next point to be able to calculate
...
fabric.util.object.extend(fabric.StaticCanvas.prototype, {
  ...
  //  alert(JSON.parse(json));
   ...
   //   _this.price_background = serialized.price_background;
...
loadJsonBack: function(json)
{
  ...
  var temp = 0;
	...
	enableLayers_2 = [];
		...
		{
			enableLayers_2 = [{'id': 0, 'name': 'Layer 1'}];
		}
		
      for(var i = 0; i< serialized.objects.length; i++){
		   listGroup2[i].enable = true;
		  var temp1 = i + 1;
		  enableLayers_2.push({'id': i , 'name': 'Layer ' + temp1});
	  }
	
	 
},
loadJsonFront: function(json)
{
	  if (!json) return;
	...
	enableLayers_1 = [];
      for(var i = 0; i< serialized.objects.length; i++){
		  ...
		  var temp1 = i + 1;
		  enableLayers_1.push({'id': i , 'name': 'Layer ' + temp1});
	  }
	
},
*/

  loadFromJSON_api: function (json, callback) {
    if (!json) return;
    var serialized = JSON.parse(json);
	//console.log(serialized);
	...
	//	var temp = 0;
	 ...
	 /*  for(var i = 0; i< serialized.objects.length; i++){
      ...
      for(var i = 0; i< temp + 1; i++){
		   //console.log(serialized.objects.length);
	//	addPriceObject(serialized.objects[i].path.length);
		  //addPriceFreeDrawing(serialized.objects[i].path);
		  var temp1 = i + 1;
		  enableLayers_1.push({'id': i , 'name': 'Layer ' + temp1});
      } */
	 // console.log(enableLayers_1);
	// console.log(listGroup1.length);
	...
	//console.log(enableLayers_1.length);
	// console.log(enableLayers_1);
	...
	//	  enableLayers_2 = [];
	 ...
	 /*
      ...
      for(var i = 0; i< temp + 1; i++){
		 // console.log(serialized.objects[i].path.length);
		//   for(var j = 0; j< serialized.objects[i].path.length; j++){
		 	//addPriceFreeDrawing(serialized.objects[i].path[j]);
		 //  }
		  var temp1 = i + 1;
		  enableLayers_2.push({'id': i , 'name': 'Layer ' + temp1});
   ...
   //   _this.price_background = serialized.price_background;
	 // convertPrice(12);
	 // convertPrice(serialized.price_background);
  ...
  fabric.Object = fabric.util.createClass(/** @scope fabric.Object.prototype */ {
     ...
     */
     stateProperties:  ('top left width height scaleX scaleY flipX flipY ' +
                      ...
                      'selectable groupLayer nameLayer posLayer price isbackground').split(' '),
   ...
   toObject: function() {
    ...
    drawBorders: function(ctx) {
      ...
      if (this.hasRotatingPoint && !this.hideCorners && !this.lockRotation && this.hasControls) {
    ...
    drawCorners: function(ctx) {
      ...
      //var img = new Image();
      //img.src = 'http://beeadesigner.dev/media/bss_design/move.gif';
      
      _left = left - scaleOffsetX;
      _top = top - scaleOffsetY;
      ctx.fillRect(_left, _top, sizeX, sizeY);
      //ctx.drawImage(img, _left - 10, _top - 10);
